...
from app.api.v1.users.requests import (
...
from app.api.v1.users.responses import (
...
from app.modules.auth.exceptions import SessionNotFoundError
...
from app.modules.users.service import ProfileContext, UserService, account_status, display_username
from app.shared.exceptions.base import ForbiddenError
from app.shared.security.device import describe_device
...
async def _resolve_listing_scope(
    ...
    (session_tenant_id) — anyone else (plain member/viewer, or no tenant
...
async def get_my_profile(
    ...
    context = await user_service.get_profile_context(current_user.id, session_tenant_id)
...
async def list_users(
    ...
    organization = await _resolve_listing_scope(current_user, session_tenant_id, org_service)
...
async def activate_user(
    ...
    user = await user_service.set_active(user_id, is_active=True, actor_id=admin.id)
    return UserOut.model_validate(user)
...
async def deactivate_user(
    ...
    user = await user_service.set_active(user_id, is_active=False, actor_id=admin.id)
    return UserOut.model_validate(user)
...
async def remove_user_permissions(
    ...
    an unknown *code* is)."""
    await user_service.remove_permissions(
        user_id, payload.permissions, tenant_slug=tenant_id, actor_id=admin.id
    )
    return RemoveUserPermissionsResponse(success=True)


@router.post("/{user_id}/roles", response_model=AssignUserRoleResponse, status_code=status.HTTP_201_CREATED)
async def assign_user_role(
    ...
    """Superuser-only — assigns one of the global (platform-wide) roles
    (admin/member/guest) to a user, addressed by role code rather than
    role_id. Same underlying RoleService.assign_role as the more general
    POST /roles/{role_id}/assignments."""
    role = await role_service.get_by_code(payload.role)
    await role_service.assign_role(user_id=user_id, role_id=role.id, assigned_by=admin.id)
    return AssignUserRoleResponse(user_id=str(user_id), role=role.code, message="Role assigned successfully")


@router.delete("/{user_id}/roles/{role_id}", response_model=RemoveUserRoleResponse)
async def remove_user_role(
    ...
    """Superuser-only counterpart to POST .../roles — revokes a global role
    assignment (see RoleService.revoke_role)."""
    await role_service.revoke_role(user_id=user_id, role_id=role_id)
    return RemoveUserRoleResponse(success=True, message="Role removed successfully")


@router.post(
    "/{user_id}/tenants", response_model=AddUserToTenantResponse, status_code=status.HTTP_201_CREATED
)
async def add_user_to_tenant(
    user_id: uuid.UUID,
    payload: AddUserToTenantRequest,
    current_user: User = Depends(get_current_user),
    org_service: OrganizationService = Depends(get_organization_service),
) -> AddUserToTenantResponse:
    """Adding someone else requires being that tenant's owner or a
    superuser — same policy as POST /organizations/{organization_id}/members,
    just addressed by tenant_id (org slug) under /users instead."""
    ...
    if role is None:
        raise RoleNotFoundError(f"No role '{payload.role}' exists in this organization")
    await org_service.add_member(
        organization.id, user_id=user_id, role_code=role.code, invited_by=current_user.id
    )
    return AddUserToTenantResponse(success=True, tenant_id=organization.slug)
...
async def delete_user(
    ...
    await user_service.delete(user_id, actor_id=admin.id)
...
async def remove_user_from_tenant(
    ...
    if user_id != current_user.id and not current_user.is_superuser:
        await org_service.require_owner(organization.id, current_user.id)
    await org_service.remove_member(organization.id, user_id, actor_id=current_user.id)
