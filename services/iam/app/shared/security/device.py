"""Genuinely cross-domain stateless helper — used by more than one module
(today: both auth and users read `describe_device` for session listings).
A helper used by exactly one module belongs in that module's own utility
file instead.
"""
...
def describe_device(user_agent: str | None) -> str:
    """Best-effort, dependency-free "<browser> on <OS>" label for a raw
    User-Agent string — purely a display convenience for session listings,
    never used for any security decision (fingerprinting/authorization stays
    off the parsed result on purpose; spoofing a UA is trivial)."""
    if not user_agent:
